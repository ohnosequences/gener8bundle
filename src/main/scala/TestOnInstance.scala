...
object TestOnInstance {
  def runTestInstance(ec2: EC2, instanceSpecs: InstanceSpecs, cmds: List[Seq[String]]): Int = {
    ...
    //     user = "ec2-user",
    //     keyfileLocations = instanceSpecs.keyName
    //   )
    // )
    // SSH(addr, sshConfig) { client => 
      println("Preparing necessary for testing tools:")
